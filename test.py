...
# ── Read Image ──────────────────────────────
image = cv2.imread(r"C:\Users\Student\Pictures\opencvimages1.jpg")

image =cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

# ── A. Brightness ───────────────────────────
bright = cv2.add(image,
         np.ones(image.shape, dtype=np.uint8) * 96)
dark   = cv2.subtract(image,
         np.ones(image.shape, dtype=np.uint8) * 50)

# ── B. Flip ─────────────────────────────────
h_flip = cv2.flip(image, 1)   # horizontal
v_flip = cv2.flip(image, 0)   # vertical
